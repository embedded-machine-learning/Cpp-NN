...
auto LinearAttention = layers::LinearAttention<UsedType,dimension_k,dimension_v,d_model>();

constexpr auto memory_buffer    = decltype(LinearAttention)::template memory_buffer<InputMatrixType>;
...
using OutputMatrixType = decltype(LinearAttention)::OutputMatrix<InputMatrixType>;


using QWeightMatrixType = decltype(LinearAttention)::QWeightMatrixType_;
using KWeightMatrixType = decltype(LinearAttention)::KWeightMatrixType_;
using VWeightMatrixType = decltype(LinearAttention)::VWeightMatrixType_;
...
auto forward(pybind11::array_t<Type> input) {
    ...
    LinearAttention(inputMatrix,output,buffer, permanent);
...
void set_QWeight(pybind11::array_t<Type> weight){
    ...
    matrixAssign(LinearAttention.QWeightMatrix, QWeightMatrix);
}

KWeightMatrixType KWeightMatrix;
void set_KWeight(pybind11::array_t<Type> weight){
    convertToBaseMatrix(weight, KWeightMatrix);
    matrixAssign(LinearAttention.KWeightMatrix, KWeightMatrix);
}

VWeightMatrixType VWeightMatrix;
void set_VWeight(pybind11::array_t<Type> weight){
    convertToBaseMatrix(weight, VWeightMatrix);
    matrixAssign(LinearAttention.VWeightMatrix, VWeightMatrix);
}

auto get_memory_info() {
    return std::make_tuple(
        decltype(LinearAttention)::template memory_buffer<InputMatrixType>,
        decltype(LinearAttention)::template memory_minimal<InputMatrixType>
    );
}


PYBIND11_MODULE(LinearAttention, m) {
