...
# print([type(weights_dict[key]) for key in weights_dict.keys()])
input_size=8
output_size=8
hidden_size=24
weights_dict = {'seq_midi.2.s5.seq.C_bias'       :  torch.ones((output_size,2)),
    'seq_midi.2.s5.seq.C'            :  torch.ones((output_size, hidden_size,2)),
    'seq_midi.2.s5.seq.norm'         :  -10*torch.ones((hidden_size)),
    'seq_midi.2.s5.seq.angle'        :  torch.randn((hidden_size))        ,
    'seq_midi.2.s5.seq.B2'           :  torch.ones((hidden_size,input_size,2)),
    'seq_midi.2.s5.seq.B2_bias'      :  torch.ones((hidden_size,2)),        
    'seq_midi.2.skipLayer.weight'    :  torch.ones((output_size,input_size)),        
}
...
class SequenceLayer(torch.nn.Module):
    ...
    def convert_to_discrete(self, step_scale=1):
        ...
        B_c = as_complex(self.B2)
        B_bias_c = as_complex(self.B2_bias)
    ...
    def forward(self, x):
        ...
        # Bu = xc@self.B_bar.T + self.B_bias_bar
        # state = Bu.clone()
        # for i in range(1,x.shape[1]):
        #     state[:,i,:] = self.Lambda_bars*state[:,i-1,:]+Bu[:,i,:]
        # y = state@self.C_bars.T + self.C_bias_bars
        # y = y.real
        # y = torch.nn.functional.leaky_relu(y)
        # if self.SkipLayer is not None:
        #     y = y + x@self.SkipLayer.T
        # return y
        xc = x.to(torch.complex64)
        state = torch.zeros(x.shape[0], self.B_bar.shape[0], dtype=torch.complex64)
        y = torch.zeros(x.shape[0], x.shape[1], self.C_bars.shape[0])
        for i in range(0,x.shape[1]):
            Bu = xc[:,i,:]@self.B_bar.T + self.B_bias_bar
            state = self.Lambda_bars*state+Bu
            y_tmp = state@self.C_bars.T + self.C_bias_bars
            y_tmp = y_tmp.real
            y_tmp = torch.nn.functional.tanh(y_tmp)
            if self.SkipLayer is not None:
                y_tmp = y_tmp + x[:,i,:]@self.SkipLayer.T
            y[:,i,:] = y_tmp        
        return y
...
with open(f'{current_pos}/weights.inc', "w") as file:
    ...
    # for i in range(2, 5):
    key = f'seq_midi.2.s5.seq'
    layers.append(SequenceLayer(weights_dict.get(key + '.norm', None), weights_dict.get(key + '.angle', None), weights_dict.get(key + '.B2', None), weights_dict.get(
        key + '.B2_bias', None), weights_dict.get(key + '.C', None), weights_dict.get(key + '.C_bias', None), weights_dict.get(f'seq_midi.2' + '.skipLayer.weight', None)))
...
with open(f'{current_pos}/network.hpp', "w") as file:
    ...
    for index in range(len(layers)):
        if layers[index].SkipLayer is not None:
            ...
            string += f'    layers::Sedge<float,Complex<float>>(A{index}, B{index}, B{index}_bias, C{index}, C{index}_bias, SkipLayer{index}_weights, FastTanh<float>),\n'
            ...
            # layers::S5_class_hidden(A, B, C, BiasRNN, BiasOut, Passthrough),
            # string += f'    layers::SSMPiano<float>(A{index}, B{index}, B{index}_bias, C{index}, C{index}_bias, FastTanh<float>),\n'
...
subprocess.run(f"{compiler} -march=native -march=native -Ofast -Wpedantic -shared -std=c++20 -fconstexpr-steps=100000000 -fPIC $(python3 -m pybind11 --includes) -I {current_pos}/../../ {current_pos}/test_paino_ssm.cpp -o  {current_pos}/CppPianoSSM.so",
...
inputs = np.random.randn(1, 44100*4, 8).astype(np.float32)

# lenght = 4*24000//upsampling
# lenght = 44100//upsampling
lenght = CppPianoSSM.getInputSize()
...
#     input_slice = input_slice_tmp
    ...
    outputs.append(tmp)
    print(tmp.shape)
    in_torch = torch.tensor(input_slice)
    start = time()
    for i,_ in enumerate(layers):
        in_torch = layers[0](in_torch)
    print("Torch time: ", time()-start)
    print("Max diff: ", np.max(np.abs(in_torch.detach().cpu().numpy()-tmp)))
    print("Mean diff: ", np.mean(np.abs(in_torch.detach().cpu().numpy()-tmp)))
    print("Output shape: ", in_torch.shape)
    print("Output shape C++: ", tmp.shape)
    outputs_torch.append(in_torch.detach().cpu().numpy())
    # assert np.allclose(out_torch.detach().cpu().numpy(), tmp, atol=1e-5)
    # break
# S5_out,classes = CppS5.run_S5(inputs)
outputs = np.concatenate(outputs, axis=1)
outputs_torch = np.concatenate(outputs_torch, axis=1)
...
plt.plot(outputs[0, ...,0], label='cpp')
plt.plot(outputs_torch[0, ...,0],alpha=0.5, label='torch')
plt.plot(outputs[0, ...,0] - outputs_torch[0, ...,0], label='diff')
plt.show()
